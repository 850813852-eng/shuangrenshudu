const { buildRoomState, decodeBoard, joinRoom, validateMove, validateSubmission } = require("./game");
const { createAndSaveRoom, loadRoom, saveRoom, withRoomLock } = require("./store");

async function createRoomHandler(name, difficulty, mode) {
...
async function moveHandler(roomCode, playerId, row, col, value) {
  const code = String(roomCode || "").trim().toUpperCase();
  const playerIdValue = String(playerId || "").trim();
  ...
  return withRoomLock(code, async () => {
    ...
    const result = validateMove(room, player, Number(row), Number(col), Number(value));
    await saveRoom(room);
