...
async function loadRoom(code) {
  ...
  const raw = await kvCommand("GET", roomKey(code));
...
async function saveRoom(room) {
  ...
  await kvCommand("SET", roomKey(room.code), JSON.stringify(room), "EX", String(ROOM_TTL_SECONDS));
}
...
async function acquireLock(code, ttlSeconds = 5) {
  ...
  while (Date.now() - startedAt < ttlSeconds * 1000) {
    const result = await kvCommand("SET", lockKey(code), token, "NX", "EX", String(ttlSeconds));
    if (result === "OK") {
      return {
        release: async () => {
          const current = await kvCommand("GET", lockKey(code));
          if (current === token) {
            await kvCommand("DEL", lockKey(code));
          }
        }
      };
    }
